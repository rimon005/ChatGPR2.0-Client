...
function App() {
  ...
  const mutation  = useMutation({
    ...
    onSuccess : (data) => setChat((prev) => [...prev, {sender : 'ai' , massage : data.massage}])
  })

  const sendMassage =async (massage) => {
    await Promise.resolve(setChat((prev) => [...prev , massage]))
    mutation.mutate();
  }
    ...
    <div className="bg-[#1A232E] h-screen py-6 relative sm:px-16 px-12 text-white overflow-hidden flex flex-col justify-between align-middle">
      ...
      <div className='gradient-02 z-0 absolute '></div>
       ...
       <div className='w-full max-w-4xl min-w-[20rem] self-center'>
        <ChatInput sendMassage={sendMassage} loading={mutation.isLoading} />
       </div>
